...
async function domoves() {
   // return;
    ...
    const query = { $or: [{}, {}]};
    query.$or[0][`weeks.${week-2}.cflteam`] = { $gt: 0 };
    ...
    await fs.writeFileSync('./lib/data/picks.json', JSON.stringify(allpicks, null, 2));
    await fs.writeFileSync('./lib/data/moves.json', JSON.stringify(moves, null, 2));
