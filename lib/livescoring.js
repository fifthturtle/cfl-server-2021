...
function getStats() {
  const activeGames = _.filter(nflschedule, x => x.status !== "game_closed" && x.status !== 'pre_game');
  if (!activeGames.length) {
    ...
    const upcoming = _.chain(nflschedule)
                      ...
                      .value();
    if (!upcoming.length) {
      scheduler.updateScoringTime(-1, -1, -1)
    } else {
      const d = new Date(upcoming[0].time);
      scheduler.updateScoringTime(d.getDay(), d.getHours(), d.getMinutes() - 1);
    }
    return;
  }
