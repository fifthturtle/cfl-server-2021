...
async function updatePlayers() {
    ...
    axios.get(url)
        .then(res => {
            ...
            //                   .map(x => {
            //                       const d = x.attributes;
            //                       const position = d.position === "TE" ? "WR" : d.position;
            //                       const _id = d.legacyIds.fantasyPlayerId;
            //                       const team = _.find(teams, t => t.lgId === (x.relationships.nflTeam.data && x.relationships.nflTeam.data.id));
            //                       const nflteam = team && team._id || 0;
            //                   .value();
            // console.log('players', players);
            //mongo.inesertMany('players', players);
        })
}
